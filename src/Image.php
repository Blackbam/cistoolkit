...
class Image
{
    ...
    public function __construct(string $imageserverUrl)
    {
        if (!filter_var($imageserverUrl, FILTER_VALIDATE_URL)) {
            throw new InvalidParameterException("Invalid Imageserver URL given to class constructor.");
        }
    ...
    public function generateSrcset(
        ...
        $srcset_values = $this->generateSrcsetValue(
            $src,
            $wh_relation,
            $step,
            $device_pixel_mode,
     ...
     */
    public function generateImageset(
        string $src,
        float $wh_relation = -1,
        $step = 2048,
        bool $device_pixel_mode = true,
        int $q = 80,
        int $zc = 1,
        string $imgsrvParamAdd = ""
    ): string {
